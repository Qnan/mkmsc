...
template <typename T>
class DLLEXPORT _SessionLocalContainer {
public:
   T& getLocalCopy (void)
   {
   ...
   T& getLocalCopy (const qword id)
   {
      ...
      return _map.findOrInsert(id);
   }

private:
   typedef RedBlackObjMap<qword, T> _Map;
...
template <typename T>
class DLLEXPORT _ReusableVariablesPool {
public:
...
template <typename T>
class DLLEXPORT _ReusableVariablesAutoRelease {
public:
   _ReusableVariablesAutoRelease () : _var_pool(0) {}
